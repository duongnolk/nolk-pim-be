...
def filter(params):
    ...
    try:
        with conn.cursor() as cursor:
            countq = "SELECT COUNT(*) FROM pim_prod.import_sku where TRUE"
            # Fetch the paginated records
            query = "SELECT * FROM pim_prod.import_sku where TRUE"

            attr_name = params.get('attr_name', None)

            # Add filters dynamically
            if attr_name:
                query += " AND attr_name LIKE %s"
                countq += " AND attr_name LIKE %s"
                query_params.append(f"%{attr_name}%")
                count_params.append(f"%{attr_name}%")
