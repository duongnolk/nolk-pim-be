...
def handler(event, context):
    ...
    if path == '/brand/filter':
        return filter(query_params)
    
    if path == '/brand/download':
        return download_csv_by_filter(query_params)
...
def filter(params):
    ...
    try:
        with conn.cursor() as cursor:
            countq = "SELECT COUNT(*) FROM pim_prod.brand where TRUE"
            # Fetch the paginated records
            query = "SELECT * FROM pim_prod.brand where TRUE"
            
            name_filter = params.get('name', None)
            
            # Add filters dynamically
            if name_filter:
                query += " AND name LIKE %s"
                countq += " AND name LIKE %s"
            ...
            query += " ORDER BY brand_id LIMIT %s OFFSET %s"
...
def download_csv_by_filter(params):
    query_params = []
    conn = getConnection()
    try:
        with conn.cursor() as cursor:
            ...
            df_result = pd.DataFrame(records, columns=eval('df_columns.' + 'brands'))
            json_result = df_result.to_json(orient='records')


            csv_output = get_csv_from_json(json_result)
